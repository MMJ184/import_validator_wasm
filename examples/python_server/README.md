...
Flask server that validates CSV **and** Excel (`.xlsx`) uploads through the
ImportValidator **native library** (Rust cdylib, called via the
`bindings/python` `ctypes` wrapper — no WASM, no Node).

What it shows:

- `POST /validate` — CSV multipart upload → `import_validator.validate_bytes`
- `POST /validate/json` — CSV as base64 in a JSON body
- `POST /validate-xlsx` — `.xlsx` multipart upload →
  `import_validator.validate_xlsx_bytes`. The workbook is buffered fully in
  memory: XLSX is a ZIP container whose central directory sits at the end of
  the file, so the engine needs random access over the complete buffer.

## Prerequisites

Python 3.9+, Rust toolchain. From the **repo root**:
