...
export function initWasm(wasmUrl) {
    if (!wasmReady)
        wasmReady = initValidatorWasm(wasmUrl);
    return wasmReady;
}
async function requireMod() {
    if (!wasmReady)
        throw new Error("WASM not initialized. Call initWasm(wasmUrl) first.");
    return await wasmReady;
}
export class Engine {
    constructor() {
        this.cachedSchemaColumns = null;
        this.cachedInputColumns = null;
    }
    static async create(schema, maxErrors, emitNormalized) {
    ...
    pushChunk(chunk, finalChunk) {
        const value = this.engine.push_chunk(chunk, finalChunk);
    ...
    static async errorCodeToString(code) {
        const mod = await requireMod();
        return mod.ValidatorEngine.error_code_to_string(code);
    }
    inputColumns() {
        if (this.cachedInputColumns)
            return this.cachedInputColumns;
        const cols = JSON.parse(this.engine.input_columns_json());
        // input columns become available after header parse; cache once non-empty.
        if (cols.length)
            this.cachedInputColumns = cols;
        return cols;
    }
    ...
    static errorCodeToStringSync(code) {
        switch (code) {
    ...
    static makeMessage(row, codeString, col) {
